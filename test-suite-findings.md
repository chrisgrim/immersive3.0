...
- **50 suspected bugs** logged below (3 high, 20 medium, 28 low). **✅ Fixed 2026-05-29: H1, H2, H3, M1, M2, M3, M4, M6, M7, M9** (L28 added while fixing H1). **M8 confirmed real but latent (NOT NULL `description` + skip-when-absent validation → masked 500); deferred pending decision.** **M5 re-classified as not-a-bug** (soft-delete intentionally retains images for restore; force-delete cleans up).
