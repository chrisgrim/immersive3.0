...
- **H-S1. Curators can mass-assign post `status` and reparent posts cross-community.** `app/Actions/Curated/PostActions.php:51` does `$request->except(['image','deleteImage'])`; `Post::$fillable` includes `status`, `community_id`, `user_id`, `shelf_id`. A curator in community A can POST a request that moves any post into their community. **Fix:** whitelist (`$request->only(['name','blurb','shelf_id','order'])`) + add `can:update,post` policy on the route.
- **H-S2. Cards modifiable across post/community boundaries (IDOR + mass-assignment).** Route `/communities/{community}/posts/{post}/cards/{card}` (`routes/curated.php:91-96`) doesn't scope route bindings. A curator can update any card by ID, including reparenting it via `Card::$fillable`'s `post_id`. **Fix:** scoped route bindings (`->scoped(['card' => 'post'])`) plus whitelist update fields in `app/Actions/Curated/CardActions.php:48`.
- **H-S3. SSRF in `EventScraperController`.** `app/Http/Controllers/Api/EventScraperController.php:31-36` validates URL syntax only; the `test` endpoint at `:139-150` has zero validation. `app/Services/EventScraper/Scrapers/GenericAIScraper.php:132-138` runs `Http::get($url)` with no host allow-list. Moderator (or hijacked moderator session) can probe `http://169.254.169.254/latest/meta-data/`, internal services, ES cluster state. **Fix:** parse host, reject private/loopback ranges (RFC1918, link-local, loopback, IPv6 equivalents), enforce `https?://`, no-redirects-without-revalidation.
- **H-S4. OAuth callbacks auto-link by email — account takeover risk.** `app/Http/Controllers/Auth/SocialAuthController.php:32-40, 92-100, 137-145` finds users by `email` and logs them in regardless of provider or verification. GitHub returns unverified primary emails for some accounts. Anyone who signs up at GitHub with a victim's email can log in as them. **Fix:** require provider-verified email (GitHub: extra `user:email` scope + emails endpoint check for `primary && verified`); if existing user has a different `provider`, refuse and surface a settings-page link flow instead of silently overwriting.
- **H-S5. XSS — unsanitized `v-html` in `card-edit.vue:380`.** Same file sanitizes at line 140 but missed line 380. **Verified.** Curator-author-only first, but as soon as a second curator/admin opens the same edit screen, it's session XSS. **Fix:** `v-html="sanitizeBlurb(card.blurb)"`.
- **H-S6. Hardcoded GeoNames API username (`chgrim`) in client bundle.** `resources/js/PageComponents/Creation/Core/Pages/location.vue:533` — username is visible in `public/build/assets/review-*.js`. Quota-scrapeable; once exhausted, location autofill breaks site-wide. **Fix:** proxy through a Laravel route that adds the username server-side.
