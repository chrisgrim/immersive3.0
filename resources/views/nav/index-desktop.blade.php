<nav
    {{-- This partial is shared between the homepage (where nav-search.vue's own
         collapsed = ref(!isHomePage()) starts expanded) and other pages that
         also include it (help/terms/search results, where it starts
         collapsed) — the CSS fallback below (used only until Vue mounts and
         overwrites --nav-bar-height) has to match whichever of those this
         particular request actually is, or every non-homepage load flashes
         from the homepage's tall nav down to the short one on first paint. --}}
    style="height: var(--nav-bar-height, {{ request()->is('/') ? '19.95rem' : '8rem' }}); transition: height 320ms cubic-bezier(0.2, 0.8, 0.2, 1); overflow-anchor: none;"
    class="index-nav">
    <div class="w-full mx-auto h-32 z-40 fixed top-0 left-0 right-0 bg-white">
        <div class="mx-auto relative h-full grid gap-0 items-center max-w-screen-5xl
            ...
            <div class="col-span-1 inline-block relative leading-none z-30 flex justify-self-start">
