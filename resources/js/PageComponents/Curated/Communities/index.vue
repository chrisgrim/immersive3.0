                        ...
                        <div class="w-full relative block overflow-hidden mb-8 rounded-xl md:flex md:h-[45rem]">
                                ...
                                <div class="w-full">
                                    ...
                                    <template v-if="isPublic(community)">
                                        <a :href="`/communities/${community.slug}/edit`">
                                            <button class="bg-white border-none p-4 rounded-l-xl hover:bg-black hover:text-white transition-colors duration-200"> 
                                                Edit 
                                            </button>
                                        </a>
                                        <a :href="`/communities/${community.slug}`">
                                            <button class="bg-white border-none p-4 rounded-r-xl hover:bg-black hover:text-white transition-colors duration-200"> 
                                                View 
                                            </button>
                                        </a>
                                    </template>
                            ...
                            <div class="relative inline-block bg-slate-400 md:w-3/5 md:h-[45rem] after:absolute after:left-0 after:top-0 after:inline-block after:h-full after:w-full after:bg-gradient-to-r from-black via-transparent to-transparent">
                                <picture>
                                    <source 
                                        type="image/webp" 
                                        :srcset="`${imageUrl}${community.largeImagePath}?timestamp=${community.updated_at}`"> 
                                    <img 
                                        loading="lazy" 
                                        class="object-cover align-bottom w-full h-full"
                                        :src="`${imageUrl}${community.largeImagePath.slice(0, -4)}jpg?timestamp=${community.updated_at}`" 
