    ...
    <div class="flex justify-end">
        <div class="px-8 md:px-32 w-full ml-[-2rem] mt-20 md:mt-12">
            ...
            <div class="w-full flex items-center justify-between mb-8">
                <!-- Title that hides when search is shown on mobile -->
                <h2 class="font-medium md:block" :class="{ 'hidden': showSearch && isMobileView }">Select Organizer</h2>
                
                <div 
                    :class="{ 'w-full': showSearch }"
                    class="flex items-center gap-4 md:w-auto">
                    <!-- Search Input - Always visible on desktop, conditionally visible on mobile -->
                    <div v-if="showSearch || !isMobileView" class="w-full md:w-[300px]">
                        <div class="flex border border-black rounded-full items-center">
                            <div class="relative flex-grow">
                                <input 
                                    ...
                                    class="w-full px-6 h-20 bg-transparent border-none"
                                    @input="handleSearch"
                                >
                                <button
                                    v-if="searchQuery || isMobileView"
                                    @click="clearSearch"
                    ...
                    <!-- Search Button - Only visible on mobile when search is not shown -->
                    <button 
                        v-if="isMobileView && !showSearch"
                        @click="showSearch = true"
                        class="flex-shrink-0 cursor-pointer rounded-full bg-gray-100 w-20 h-20 flex items-center justify-center hover:bg-gray-200 md:hidden"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-8 h-8">
...
// Setup mobile detection on mount and fetch teams
onMounted(() => {
    fetchTeams()
    checkMobileView()
...
const checkMobileView = () => {
    isMobileView.value = window.innerWidth < 768 // Matches Tailwind's md breakpoint
}
...
const clearSearch = () => {
    ...
    // On mobile, hide the search input when clearing
