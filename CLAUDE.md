...
- **Endpoint** `/mcp` (`routes/ai.php`, server `app/Mcp/Servers/EiServer.php`, 13 tools under `app/Mcp/Tools/`). Auth is **OAuth 2.1 via Laravel Passport** (`api` guard): a client discovers the flow from the 401's `WWW-Authenticate`, registers itself (`POST /oauth/register`, redirect domains allowlisted in `config/mcp.php`), sends the user to `/oauth/authorize` (`routes/oauth.php`, our consent view `resources/views/oauth/authorize.blade.php`), exchanges the code with PKCE at `/oauth/token`. Personal access tokens from the API keys page work the same way for scripts. Sanctum is SPA-cookie-only now; bearer tokens never authenticate `auth:sanctum` routes.
...
- Event search index only includes published events (`shouldBeSearchable()` checks `status === 'p'`). Since 2026-09-13 every *explicit* reindex goes through `Event::syncSearchIndex()` (plain saves still go through Scout's observer, one job per save) and `UpdateEventAction` batches them with `Event::deferringSearchSync()` (one index update per save, after commit; enter it at transaction level 0, see its docblock); `SCOUT_QUEUE=true` on the servers makes each update a queued `MakeSearchable`/`RemoveFromSearch` job for the `ei-queue*` workers. If Elasticsearch is down for a while those jobs fail 3× into `failed_jobs`: run `php artisan queue:retry all` once it is back. Reads go through `App\Support\Search\SearchGuard` so the search page and nav autocomplete return empty results (+ `search_unavailable: true`) instead of 500 while the cluster is unreachable.
